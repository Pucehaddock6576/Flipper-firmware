...
static void send_and_show(App* app, const char* label, const char* cmd) {
...
static void submenu_cb(void* context, uint32_t index) {
    ...
    switch(index) {
    case IdxScanBle:
        send_and_show(app, "Scanning BLE devices...", "scanbt\n");
        break;
    case IdxDetectSkimmers:
        send_and_show(app, "Detecting card skimmers...", "scanskimmers\n");
        break;
    case IdxDetectAirtags:
        send_and_show(app, "Detecting AirTags...", "sniffbt -t at\n");
        break;
    case IdxScanFlippers:
        send_and_show(app, "Scanning Flipper devices...", "scanbt -t flipper\n");
        break;
    case IdxStopScan:
        send_and_show(app, "Stopping scan...", "stopscan\n");
        break;
    }
}
...
static App* app_alloc(void) {
    ...
    app->worker_thread = furi_thread_alloc_ex("BtScanRx", 1024, uart_worker, app);
    ...
    submenu_set_header(app->submenu, "BT Scanner");
    submenu_add_item(app->submenu, "Scan BLE Devices", IdxScanBle, submenu_cb, app);
    submenu_add_item(app->submenu, "Detect Card Skimmers", IdxDetectSkimmers, submenu_cb, app);
    submenu_add_item(app->submenu, "Detect AirTags", IdxDetectAirtags, submenu_cb, app);
    submenu_add_item(app->submenu, "Scan Flipper Devices", IdxScanFlippers, submenu_cb, app);
    submenu_add_item(app->submenu, "Stop Scan", IdxStopScan, submenu_cb, app);
...
int32_t phantom_bt_scanner_app(void* p) {
    UNUSED(p);
    App* app = app_alloc();
